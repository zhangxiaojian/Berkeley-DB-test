...
int main()
{
	...
	try{
		...
		std::cout<<*((int*)pkey1.get_data())<<" : "<<(char*)tempData1.get_data()<<std::endl
		     	<<*((int*)pkey2.get_data())<<" : "<<(char*)tempData2.get_data()<<std::endl;
		//txn2 to modify 
		DbTxn *txn2 = NULL;
		myEnv.txn_begin(NULL,&txn2,0);
		//modify the first record with the next record's data
		dbp->put(txn2,&pkey1,&tempData2,0);     //after try, this put method will not wait ..for read_committed degree
		//dbp->put(txn2,&pkey2,&tempData1,0);   //this method will block for the cursor still point to that record

		//close cursor before txn
		cursorp->close();
		//commit the txn
		txn1->commit(0);
		txn2->commit(0);
 ...
 * 5.After excute , we should use DB_QUEUE access method for it has record-level lock
 */
